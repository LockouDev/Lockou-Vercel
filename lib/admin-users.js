...
export function sanitizeAdminUser(user, permissions = []) {
  ...
  return {
    ...
    robloxOauthStatus: user.robloxOauthStatus || "",
    robloxOauthDecidedAt: user.robloxOauthDecidedAt || ""
  };
}
...
export async function updateAdminRobloxProfile(userId, profile) {
  ...
  const nextUser = {
    ...
    robloxOauthStatus: "connected",
    robloxOauthDecidedAt: new Date().toISOString()
  };
...
export async function updateAdminRobloxOauthDecision(userId, status) {
  const user = await readAdminUserById(userId);
  ...
  const nextUser = {
    ...user,
    robloxOauthStatus: String(status || "").trim(),
    robloxOauthDecidedAt: new Date().toISOString()
  };
